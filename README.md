...
5. 已支持 GitHub Action，首先 Fork 项目，在 Fork 的项目的 Secrets 中添加
CHECKIN_TITLE(签到机场名称)
CHECKIN_URL（签到地址） 
CHECKIN_EMAIL（注册邮箱）
CHECKIN_PASSWORD（注册密码）
四个数据，不同的账号以&分隔，定时和 Star 自己的项目均可运行。关于如何保持同步请参考[pull](https://github.com/wei/pull/blob/master/README.md)。
...
感谢：@evilbutcher https://github.com/evilbutcher/Quantumult_X/tree/master
...
Chrome插件
Astar VPN
https://chrome.google.com/webstore/detail/astar-vpn-free-and-fast-v/jajilbjjinjmgcibalaakngmkilboobh?hl=zh-CN 速度还行,用了很久,很稳定.

VeePN
https://chrome.google.com/webstore/detail/veepn-unlimited-free-fast/majdfhpaihoncoakbjgbdhglocklcgno 速度不错,登录试用全部节点一天,有免费节点,但可以刷.具体可以邮件私聊我.

SkyZIP Proxy
https://chrome.google.com/webstore/detail/skyzip-proxy/hbgknjagaclofapkgkeapamhmglnbphi?hl=zh-CN 速度不稳,时快时慢,总体还是不错,比前两个差.
